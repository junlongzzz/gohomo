...
2. 下载 [Mihomo](https://github.com/MetaCubeX/mihomo/releases) 可执行文件，并准备其配置文件 `config.yaml` 或
   `config.yml`。
...
| 配置项                | 类型            | 说明                                                               | 默认值       |
|--------------------|---------------|------------------------------------------------------------------|-----------|
| `auto-start`       | bool          | 是否开机自动启动程序 *                                                     | `false`   |                                                                 | `false`   |
| `proxy-by-pass`    | array(string) | 不走代理（直连）的地址列表，仅在系统代理模式下生效                                        | 常见私有 IP 段 |
| `proxy-mode`       | string        | 代理模式：`close` 关闭、`system` 系统代理、`tun` TUN 模式 *                     | `system`  |
| `core-log-enabled` | bool          | 是否将核心的运行日志写入文件 *                                                 | `false`   |
| `core-run-mode`    | string        | 核心运行模式：`rule` 规则、`global` 全局、`direct` 直连 *                       | `rule`    |
| `core-override`    | map           | 核心配置覆写，可自定义或覆盖 `Mihomo` 任意配置项，详见 [覆写规则](./docs/core-override.md) | `{}`      |

> 配置说明后面标有 `*` 表示该配置项可在 **托盘界面** 内修改（推荐）。
